...
Consider 📚 [`Iterator`]⏱2h adapter methods as an example: 📚 [`Iterator::map()`][7], 📚 [`Iterator::enumerate()`][8], 📚 [`Iterator::filter()`][9], etc. They all return a concrete adapter type, rather than `impl Iterator<..>` abstraction.
