...
The common and obvious rules in 📚 [Rust]⏱0.25h when choosing type for an input parameter are the following:
...
```
However, due to the need of explicit type conversions in 📚 [Rust]⏱0.25h, such API can lack ergonomics in use (notice the explicit conversion methods that API user has to use):
```rust
...
This is one of the key features, which drive 📚 [Rust]⏱0.25h expressiveness and ergonomics. Just look over `std` library to see how widely it's used: 📚 [`Iterator::eq()`][1]⏱2h, 📚 [`Vec::drain()`][2]⏱3.5h, 📚 [`HashMap::extend()`][3]⏱0.5h, etc.
...
- 📰 [Joe Wilm: From &str to Cow][4]⏱0.25h
- 📰 [Pascal Hertleif: Elegant Library APIs in Rust: Use conversion traits][5]⏱0.5h
- 📰 [Carl M. Kadie: Nine Rules for Elegant Rust Library APIs][10]⏱0.75h
...
Consider 📚 [`Iterator`]⏱2h adapter methods as an example: 📚 [`Iterator::map()`][7]⏱2h, 📚 [`Iterator::enumerate()`][8]⏱2h, 📚 [`Iterator::filter()`][9]⏱2h, etc. They all return a concrete adapter type, rather than `impl Iterator<..>` abstraction.
