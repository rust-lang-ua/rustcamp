...
As 🏠 [Rust] is a 📰 [strongly typed][1]⏱0.5h language, all type conversions must be performed explicitly in the code. As 🏠 [Rust] has a rich type system (programming logic and semantics are mostly expressed in types rather than in values), type conversions are inevitable in almost every single line of code. Fortunately, 🏠 [Rust] offers 📚 [well-designed type conversion capabilities][`std::convert`]⏱0.25h, which are quite ergonomic, intuitive and are pleasant to use.
...
Value-to-value conversion in 🏠 [Rust] is done with 📚 [`From`]⏱1h and 📚 [`Into`] mirrored traits (implementing the first one automatically implements another one). These traits provide __non-fallible conversion__.

If your conversion may fail, then you should use 📚 [`TryFrom`]⏱0.75h/📚 [`TryInto`] analogues, which __allow failing in a controlled way__.
...
For better understanding 📚 [`From`]⏱1h/📚 [`Into`] and 📚 [`TryFrom`]⏱0.75h/📚 [`TryInto`] purpose, design, limitations and use cases read through:
...
- 🧭 [Rust Book: 15.2. Treating Smart Pointers Like Regular References with the Deref Trait][3]⏱0.5h
- 📚 [Official `Deref` docs][`Deref`]
- 📰 [Deref vs AsRef vs Borrow vs Cow][13]⏱0.25h
...
The common temptation is to use [`Deref`] in a combination with 📰 [newtype pattern][4]⏱0.25h, so you can use your inner type via outer type without any explicit requirements. However, this is considered to be a bad practice, and 📚 [official `Deref` docs][`Deref`] clearly states:
...
For casting between types the 📚 [`as` keyword][`as`]⏱0.25h is used in 🏠 [Rust].
...
However, it supports only a 📚 [small, fixed set of transformations][7]⏱0.5h, and __is 📚 [not idiomatic][11] to use when other conversion possibilities are available__ (like 📚 [`From`]⏱1h, 📚 [`TryFrom`]⏱0.75h, 📚 [`AsRef`]).
...
5. Why using 📚 [`as`]⏱0.25h keyword is not a good practice in 🏠 [Rust]? Why do we still use it?
